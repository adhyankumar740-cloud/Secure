...
PROFANITY_REGEX = re.compile(
    r'(bh[e3]nchod|madar[c]hod|ch[u0]t[i1]ya|g[a@]nd|l[u0]nd|b[e3]h[e3]nchod|'
    r'r[a@]ndi|s[a@]l[a@]|k[a@]m[i1]n[a@]|f+u+c+k+|sh[i1]t+|b[i1]tch|'
    r'[a@]ssh[o0]l[e3]|b[e3]wk[o0]of|h[a@]r[a@]mi|g[a@]ndu|ch[u0]t|'
    r'lodu|chodu|bsdk|mc|bc|mf)',
    re.IGNORECASE
)
...
def normalize_for_profanity(text: str) -> str:
    ...
    # Remove spaces between single chars: "f u c k" -> "fuck"
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                url, json={"inputs": text},
                headers=headers,
                timeout=4.0  # increase from 3.0
            )
...
async def check_hf_multilingual(text: str) -> bool:
    """Uses multilingual model for Hindi/Hinglish text"""
    if not HF_API_KEY or not text.strip(): return False
    # This model handles Hindi, Hinglish, and mixed text
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                for r in inner:
                    if r.get('label') == 'HATE' and r.get('score', 0) > 0.72:
                        return True
        except Exception:
            pass
    return False
...
async def ingestion_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_bypass_symbols or has_scam_context or is_untrusted or threat_multiplier > 1.5:
        ...
        # TIER 2: Hugging Face Fast Toxic-BERT & Multilingual Guard Channels
        if HF_API_KEY:
            ...
            if await check_hf_multilingual(raw_payload_text):
                await db_layer.update_user_reputation(chat.id, user.id, -20.0)
        ...
        # TIER 4: Deep Autonomous Context Interpretation via Groq (Primary Filter)
