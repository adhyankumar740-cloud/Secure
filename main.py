...
from openai import AsyncOpenAI  # Groq and OpenAI use compatible clients
...
# ------------------------------------------------------------------
# CLOUD MODERATION BACKENDS (OPENAI + PERSPECTIVE)
# ------------------------------------------------------------------
async def check_tier2_openai_moderation(text: str) -> bool:
    """Tier 2: OpenAI Free Moderation API for Global Policies"""
...
async def check_perspective_toxicity(text: str) -> float:
    """Tier 2.5: Google Perspective API (Multilingual/Hinglish Core)"""
...
async def evaluate_via_groq(text: str) -> dict:
    """Uses Groq Cloud API endpoint for advanced contextual scanning."""
...
async def ingestion_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- TIER 1: INSTANT LOCAL ENGINE MATCH (Sabse Pehle Run Hoga) ---
    ...
    # 🚀 SMART SUSPECT LOGIC (LOCAL PASS HONE KE BAAD - CLOUD AI SE PEHLE)
    # Check A: Kya bypass ke liye ajeeb/uncommon symbols use kiye hain?
    has_bypass_symbols = bool(re.search(r'[^a-zA-Z0-9\s\u0900-\u097F]', raw_text))
    # Check B: Kya text me high-risk trigger keywords hain?
    has_scam_context = any(w in raw_text.lower() for w in ["crypto", "earn", "join", "channel", "airdrop", "free", "gift", "money", "invest", "deta hu"])
    # Check C: Kya lamba paragraph hai jisme dynamic analysis zaroori hai?
    is_long_paragraph = len(raw_text.split()) > 15

    # Cloud Checking sirf tabhi trigger hogi jab message suspect lagega!
    if has_bypass_symbols or has_scam_context or is_long_paragraph:
        ...
        # Normal chat messages seedhe bina kisi API delay ya hit ke safe zone me nikal jayenge!
        pass
