...
# NAYI LIBRARY IMPORT KIYA FOR LOCAL SHIELD
...
# ------------------------------------------------------------------
# LOCAL PROFANITY SHIELD (INFINITE LIMITS)
# ------------------------------------------------------------------
profanity.load_censor_words() # English default words load ho gaye

# Hinglish bad words list jo Groq tak message ko pahunchne hi nahi degi
HINGLISH_BAD_WORDS = [
    "bhenchod", "madarchod", "chutiya", "gand", "lund", "behenchod", "randi", 
    "sala", "kamina", "bc", "mc", "bhadwa", "harami", "bsdk", "gandu", 
    "lawda", "lavda", "l@uda", "chut", "bhadwe", "saala", "kamine"
]
profanity.add_censor_words(HINGLISH_BAD_WORDS)

# Baaki basic local catches
...
# ------------------------------------------------------------------
# TEXT CLEANING & ANALYSIS ENGINES
# ------------------------------------------------------------------
...
async def evaluate_via_groq(text: str) -> dict:
    """Groq API gets called ONLY for complex scan after local library filter passes."""
    if not ai_client: return {"violation": False, "action": "ignore"}
    
    prompt = (
        "You are an automated, strict backend security bot for a developer Telegram chat group. "
        "Analyze this text (can be mixed Hindi/Hinglish/English) ONLY for sneaky scams, "
        "crypto links, channel plug raids, or advanced malicious exploitation intents. "
        "Respond ONLY with a standard raw JSON structure: "
        '{"violation": true/false, "action": "ban"/"mute"/"ignore", "confidence": 0-100, "reason": "short summary"}'
    )
    try:
        ...
        logger.error(f"Groq evaluation rate-limit/error: {e}")
...
async def ingestion_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- TIER 1: INFINITE LIMIT LOCAL LIBRARY SHIELD (0 API COST) ---
    # `better-profanity` checks english default + custom hinglish words + symbol variations instantly
    if profanity.contains_profanity(raw_text) or profanity.contains_profanity(normalized_text):
        return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Abusive Language / Profanity Filtered", username)

    # Basic fast regex rules
    ...
    # --- TIER 2: GROQ AI DEEP CONTEXT EVALUATION ---
    # Groq par sirf tabhi request jayegi jab text clean hoga aur 5 char se bada hoga (Saving 90%+ Tokens!)
    if ai_client and len(raw_text.strip()) > 5:
...
async def post_startup_validation(app: Application):
    ...
    logger.info("🛡️ Multi-Tier Local Library Shield + Groq AI Core Firewall Activated.")
