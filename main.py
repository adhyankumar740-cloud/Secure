...
# Optimized Compiled Regular Expressions
...
class BehavioralProfiler:
    ...
    def calculate_account_trust_score(user_id: int, total_messages: int, reputation: float) -> tuple:
        ...
        # Account Age Inference Heuristic via Telegram Sequential ID distribution
        # High IDs (> 6,000,000,000) represent modern throwaway accounts.
        age_risk = "LOW"
...
# ------------------------------------------------------------------
# CLOUD BACKENDS (OPENAI + HF TOXIC-BERT + PERSPECTIVE)
# ------------------------------------------------------------------
async def check_tier2_openai_moderation(text: str) -> bool:
    if not openai_client or not text.strip(): return False
    try:
...
async def check_tier2_2_hf_toxic_bert(text: str) -> bool:
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(url, json={"inputs": text}, timeout=3.0)
            if response.status_code == 200:
                predictions = response.json()[0]
                for pred in predictions:
                    if pred['score'] > 0.68: return True
...
async def ingestion_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if media_unique_id:
        incident_count = await db_layer.check_duplicate_media_hash(chat.id, media_unique_id, msg.type)
    ...
    is_bot_pattern = BehavioralProfiler.analyze_structural_velocity(chat_id, user.id, raw_payload_text)
    ...
    # Smart conditions block: Cloud checking runs only if text meets high-risk characteristics
    ...
    if has_bypass_symbols or has_scam_context or is_untrusted or threat_multiplier > 1.5:
        ...
        # TIER 1: OpenAI Free Moderation Shield
        if openai_client:
            if await check_tier2_openai_moderation(raw_payload_text):
                await db_layer.update_user_reputation(chat.id, user.id, -25.0)
                return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Global Safety Core Vector Match (OpenAI)", username)

        # TIER 2: Hugging Face Fast Toxic-BERT
        if HF_API_KEY:
            if await check_tier2_2_hf_toxic_bert(raw_payload_text):
                await db_layer.update_user_reputation(chat.id, user.id, -15.0)
        ...
        # TIER 3: Google Perspective Multilingual Engine
        # Threat stress level automatically scales down acceptable toxicity triggers
        base_threshold = 0.82 if threat_multiplier == 1.0 else 0.65
        if PERSPECTIVE_API_KEY:
            tox_score = await check_perspective_toxicity(raw_payload_text)
            if tox_score > base_threshold:
                await db_layer.update_user_reputation(chat.id, user.id, -20.0)
                return await execute_local_punishment(context, chat.id, user.id, msg.message_id, f"Multi-script Lexicon Guard Trigger ({int(tox_score*100)}%)", username)

        # TIER 4: Deep Autonomous Context Interpretation via Groq
        if groq_client and len(raw_payload_text.strip()) > 8:
            ...
            if ai_res.get("violation") and ai_res.get("confidence", 0) >= 80:
                ...
                if action == "ban":
                    ...
                    await context.bot.send_message(chat_id, f"🛑 <b>Autonomous AI Defense Network Ban</b>\n<b>User:</b> {username}\n<b>Profile Trust:</b> {trust_score:.1f}%\n<b>Reason:</b> {reason}", parse_mode=ParseMode.HTML)
                elif action == "mute":
                    ...
                    await context.bot.send_message(chat_id, f"🔇 <b>Autonomous AI Defense Network Mute (2 Hours)</b>\n<b>User:</b> {username}\n<b>Reason:</b> {reason}", parse_mode=ParseMode.HTML)
...
async def gatekeeper_join_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Identify Coordinated Raid Sequences: >6 user joins within 10 seconds triggers automatic group lockdown
    ...
    if len(recent_joins) > 6:
        ...
        # Revoke group write permissions globally
    ...
    # Standard Entry Point Filtering
    for human in new_humans:
        try:
            # Check user identifier to detect fresh accounts instantly
            if human.id > 6500000000:
                # Force instant mute restriction for unverified modern endpoints
...
async def process_gate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in captcha_registry and user_id in captcha_registry[chat_id]:
        try:
            ...
            # Boot system reputation parameters positively
...
async def analytics_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    report = (
        ...
        f"• <b>Global Firewall Status:</b> <code>ONLINE (Tier 4)</code>\n"
    )
...
async def memory_janitor():
    while True:
        ...
        now = time.time()
        try:
            for cache in [flood_cache, behavioral_velocity_cache]:
                for cid in list(cache.keys()):
                    for uid in list(cache[cid].keys()):
                        cache[cid][uid] = [item if isinstance(item, tuple) else (item, 0) for item in cache[cid][uid]]
                        cache[cid][uid] = [t for t in cache[cid][uid] if now - t[0] < 30]
                        if not cache[cid][uid]: del cache[cid][uid]
                    if not cache[cid]: del cache[cid]
        except Exception: pass

# ------------------------------------------------------------------
# ENFORCEMENT & DYNAMIC ESCALATION MATRIX
# ------------------------------------------------------------------
async def execute_local_punishment(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, msg_id: int, reason: str, username: str, immediate_ban: bool = False):
    ...
    if warnings == 1:
        ...
        try:
            ...
            await context.bot.send_message(chat_id, f"🛑 <b>Permanent Ban Escalation:</b> {username}\n<b>Reason:</b> {reason} (Warnings Exhausted)", parse_mode=ParseMode.HTML)
        except TelegramError: pass
