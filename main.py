...
from unidecode import unidecode # NEW: For Advanced De-obfuscation
...
media_cache = defaultdict(lambda: defaultdict(list)) # Now used for Duplicate Media Tracking
...
# ------------------------------------------------------------------
# CLOUD MODERATION BACKENDS 
# ------------------------------------------------------------------
async def check_tier2_openai_moderation(text: str) -> bool:
...
async def check_perspective_toxicity(text: str) -> float:
    if not PERSPECTIVE_API_KEY or not text.strip(): return 0.0
...
# ------------------------------------------------------------------
# LOCAL SQLITE DATABASE LAYER (UPGRADED WITH TRUST SCORING)
# ------------------------------------------------------------------
class DatabaseLayer:
    ...
    async def init_db(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('''CREATE TABLE IF NOT EXISTS user_trust (user_id INTEGER PRIMARY KEY, score INTEGER)''') # NEW: Trust Scoring Table
            await db.commit()
    ...
    async def get_trust_score(self, user_id: int) -> int:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT score FROM user_trust WHERE user_id=?", (user_id,)) as cursor:
                row = await cursor.fetchone()
                if row: return row[0]
                
                # Heuristic: New bot accounts usually have huge 10+ digit IDs
                initial_score = 10 if len(str(user_id)) > 9 else 50
                await db.execute("INSERT INTO user_trust (user_id, score) VALUES (?, ?)", (user_id, initial_score))
                await db.commit()
                return initial_score

    async def update_trust_score(self, user_id: int, points: int):
        current = await self.get_trust_score(user_id)
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("UPDATE user_trust SET score=? WHERE user_id=?", (current + points, user_id))
            await db.commit()
...
# ------------------------------------------------------------------
# SECURITIES & TEXT REGEX ENGINES (UPGRADED)
# ------------------------------------------------------------------
def normalize_and_clean_text(text: str) -> str:
    """NEW: Implements unidecode to break Zero-width & Cyrillic bypasses"""
    if not text: return ""
    text = re.sub(r'[\u200B-\u200D\uFEFF\x00-\x1F\x7F]', '', text)
    text = unidecode(text).lower() # Convert fancy fonts (𝕱𝖗𝖊𝖊) to basic ascii (free)
            ...
            for cache in [flood_cache, media_cache]:
                for cid in list(cache.keys()):
                    for uid in list(cache[cid].keys()):
                        # Clean timestamps older than 30s
                        if isinstance(cache[cid][uid], list):
...
async def execute_local_punishment(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int, msg_id: int, reason: str, username: str, immediate_ban: bool = False):
    ...
    # Trust Score Penalty
    await db_layer.update_trust_score(user_id, -20 if immediate_ban else -5)
...
# ------------------------------------------------------------------
# NEW: ADVANCED MEDIA & ARCHIVE INSPECTION PIPELINE
# ------------------------------------------------------------------
async def media_inspection_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trust_score = await db_layer.get_trust_score(user.id)

    # 1. Trust Gate for Media (Block low trust users from sending media)
    if trust_score < 20:
        try:
            await context.bot.delete_message(chat.id, msg.message_id)
            warning = await context.bot.send_message(chat.id, f"⚠️ {username}, your Trust Score is too low to send media/files. Chat normally to increase it.")
            asyncio.create_task(delete_after(context.bot, chat.id, warning.message_id, 10))
        except TelegramError: pass
        return

    # 2. Duplicate Media Hash Tracking (Anti-Spam)
    file_id_to_hash = None
    if msg.photo: file_id_to_hash = msg.photo[-1].file_unique_id
    elif msg.video: file_id_to_hash = msg.video.file_unique_id
    elif msg.document: file_id_to_hash = msg.document.file_unique_id
    
    if file_id_to_hash:
        now = time.time()
        media_cache[chat.id][file_id_to_hash].append(now)
        # If same file sent > 3 times in 30 seconds globally in the chat
        if len(media_cache[chat.id][file_id_to_hash]) > 3:
            return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Duplicate Media Spam Loop", username)

    # 3. Deep Archive (Zip) Scanning
    if msg.document:
        doc_name = (msg.document.file_name or "").lower()
        if any(doc_name.endswith(ext) for ext in BANNED_EXTENSIONS):
            return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Malicious Extension Detected", username, immediate_ban=True)

        # Inspect ZIP files under 5MB dynamically
        if doc_name.endswith('.zip') and msg.document.file_size < 5000000:
            try:
                ...
                with zipfile.ZipFile(file_bytes) as zf:
                    for inner_name in zf.namelist():
                        if any(inner_name.lower().endswith(ext) for ext in BANNED_EXTENSIONS):
                            return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Malware Disguised inside Archive", username, immediate_ban=True)
            except zipfile.BadZipFile:
                pass # Safe to ignore if not a valid zip
            except Exception as e:
                logger.error(f"Archive scan error: {e}")

    # Pass captions to normal text ingestor
    if msg.caption:
        await ingestion_pipeline(update, context)
...
async def ingestion_pipeline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Reward user trust for normal messaging
    await db_layer.update_trust_score(user.id, 1)

    # --- TIER 1: INSTANT LOCAL ENGINE MATCH ---
    ...
    # Flood Control Logic
    if not isinstance(flood_cache[chat.id][user.id], list): flood_cache[chat.id][user.id] = []
    ...
    # 🚀 SMART SUSPECT LOGIC 
    has_bypass_symbols = bool(re.search(r'[^a-zA-Z0-9\s\u0900-\u097F]', raw_text))
    has_scam_context = any(w in raw_text.lower() for w in ["crypto", "earn", "join", "channel", "airdrop", "free", "gift", "money", "invest", "deta hu"])
    is_long_paragraph = len(raw_text.split()) > 15

    if has_bypass_symbols or has_scam_context or is_long_paragraph:
        logger.info(f"🔍 Suspect Activity Found. Routing to Cloud Shields: '{raw_text[:25]}...'")

        # --- TIER 2: OPENAI FREE MODERATION API ---
        if openai_client:
            is_openai_flagged = await check_tier2_openai_moderation(raw_text)
            if is_openai_flagged:
                return await execute_local_punishment(context, chat.id, user.id, msg.message_id, "Global Content Safety Policy Trigger (OpenAI)", username)

        # --- TIER 2.5: GOOGLE PERSPECTIVE WEB API ---
        ...
        if groq_client and len(raw_text.strip()) > 6:
            ...
            if ai_res.get("violation") and ai_res.get("confidence", 0) >= 80:
                ...
                if action == "ban":
                    ...
                    await db_layer.update_trust_score(user.id, -50)
                    await db_layer.log_action(chat_id, user.id, 0, "AI_AUTONOMOUS_BAN", reason)
                    await context.bot.send_message(chat.id, f"🛑 <b>Autonomous Groq AI Ban Executed</b>\n<b>User:</b> {username}\n<b>Reason:</b> {reason}", parse_mode=ParseMode.HTML)
                elif action == "mute":
                    until = int(time.time()) + 3600
                    await db_layer.update_trust_score(user.id, -20)
...
async def gatekeeper_join_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # RAID DETECTION UPGRADED: 6 joins in 10 seconds triggers lockdown
    if len([t for t in join_cache[chat.id] if now - t < 10.0]) > 6:
        ...
        await context.bot.send_message(chat.id, "🚨 <b>MASS RAID DETECTED!</b>\nGroup locked down for 10 minutes to protect members.", parse_mode=ParseMode.HTML)
        for h in new_humans: 
    ...
    for human in new_humans:
        try:
            ...
            await db_layer.get_trust_score(human.id)
...
def main():
    ...
    # NEW: Handle Media & Documents separately to save processing time
    app.add_handler(MessageHandler(filters.Document.ALL | filters.PHOTO | filters.VIDEO, media_inspection_pipeline))
